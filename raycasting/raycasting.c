...
/*   Updated: 2022/02/25 19:48:09 by aarchiba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_camera(t_data_mlx *data, int x)
{
	...
	data->map->camera.delta_dir_x = abs(1 / data->map->player.dir_x);
	data->map->camera.delta_dir_y = abs(1 / data->map->player.dir_y);
...
void	ray_player(t_data_mlx *data)
{
	...
	while (++x < WIDTH)
	{
		...
		if (data->map->camera.wall_dir)
			data->map->camera.correct_dist = (data->map->camera.side_dist_y - \
				data->map->camera.delta_dir_y);
		else
			data->map->camera.correct_dist = (data->map->camera.side_dist_x - \
